            ...
            try
            {
                ...
                foreach (var message in response.Messages)
                {
                    try
                    {
                        ...
                        {
                            // Not a message this listener knows. Retrying cannot change that, so it is deleted
                            // and skipped — the same as the Kafka listener, which commits it.
                            logger.LogWarning("Skipping a message that could not be parsed: {MessageId}",
                                message.MessageId);
