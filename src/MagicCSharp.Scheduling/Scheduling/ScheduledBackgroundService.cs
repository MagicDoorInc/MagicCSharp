    ...
    ILogger logger) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            // Report the next check against the same clock the scheduling uses. Reading the wall clock here
            // would print a time the service is not actually working from whenever the two differ, which is
            // exactly the situation — a test moving the clock — where the log matters most.
            logger.LogDebug("{ServiceName} sleeping for {Delay:F1} seconds until next check at {NextCheck}",
                ServiceName, delay.TotalSeconds, clock.Now().Add(delay));
