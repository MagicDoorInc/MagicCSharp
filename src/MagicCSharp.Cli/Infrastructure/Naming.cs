...
public static partial class Naming
{
    ...
    /// <summary>
    ///     PascalCase to the variable name MagicCSharp.Analyzers expects for that type (MCS0019): "OrderDal"
    ///     becomes "orderDal", and a leading acronym is lowercased as one word, so "APIKey" becomes "apiKey".
    /// </summary>
    public static string ToVariableName(string input)
    {
        var leadingUpper = 0;
        while (leadingUpper < input.Length && !char.IsLower(input[leadingUpper]))
        {
            leadingUpper++;
        }

        if (leadingUpper == input.Length)
        {
            return input.ToLowerInvariant();
        }

        // In "APIKey" the K starts the next word, so it stays upper case.
        var lowered = leadingUpper <= 1 ? leadingUpper : leadingUpper - 1;
        return input[..lowered].ToLowerInvariant() + input[lowered..];
    }
