...
The transport is one registration in `Program.cs`. `AddMagicApp()` alone dispatches in-process;
`AddMagicKafkaEvents(...)` or `AddMagicSqsEvents(...)` before it sends events through a broker instead, and the
in-process dispatcher steps aside. Publishers and handlers do not change. A service can be both producer and
consumer of its own topic, so a handler runs a moment after the request that published its event.
`project.md` says which transport this repository uses.

Tests do not need a broker: the test base swaps in `SyncEventDispatcher`, which runs handlers before `Dispatch`
returns.
