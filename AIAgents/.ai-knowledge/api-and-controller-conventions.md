...
Authentication and authorisation sit in front of the use cases, not inside them: the auth policy on the
controllers, and — once callers differ in what they may reach — a thin layer of use cases between controllers
and the business ones for access checks and caller identity. The business use cases stay callable from
background services, handlers and tests, where there is no HTTP caller. See `use-case-patterns.md`.
