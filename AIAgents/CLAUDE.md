...
```bash
dotnet build {{ prefix }}.All.slnx
dotnet test {{ prefix }}.All.slnx          # repository tests need Docker: they run against PostgreSQL
mcs validate --path .
```

A change is done when all three pass. Most work only needs one service's solution, `{{ prefix }}.{Service}.slnx`.
...
`CLAUDE.md` and the guides in `.ai-knowledge/` come from MagicCSharp and are refreshed by `mcs update
