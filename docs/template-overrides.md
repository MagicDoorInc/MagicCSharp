...
Every file `mcs` generates comes from a Scriban template — twenty-one of them. You can replace any one with
your own, and the rest keep coming from the tool. That is how a team gets house style without a fork: the
audit columns every table carries, a licence header, a test file that starts from your fixtures — one file
each, while the other twenty keep tracking upstream.
...
Because it is **per file**, overriding `Entities/dal.cs.hbs` leaves the other twenty built-in and still
