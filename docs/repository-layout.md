...
`mcs init` gives you these, and will not overwrite any that already exist:

| File | Why |
|---|---|
| `magiccsharp.json` | Holds the prefix. Its presence is how every other tool knows it is at the repository root. |
| `Directory.Build.props` | Target framework, nullable, implicit usings, warnings-as-errors and the [build rules](#build-rules) — inherited by every project, so a `.csproj` carries only what is specific to it. |
...
| `AGENTS.md`, `CLAUDE.md`, `.ai-knowledge/` | The conventions written down for AI coding agents — `AGENTS.md` for every agent, `CLAUDE.md` importing it for Claude Code — one guide per topic, and `.ai-knowledge/project.md` for what is specific to your repository. `mcs update ai-files` refreshes the guides and never touches `project.md`. Not written with `--no-ai-knowledge`. |
...
|---|---|
| No direct `DateTime.Now` / `UtcNow` | The behaviour becomes untestable — you cannot test a thirty-day rule without waiting thirty days. Inject `TimeProvider`. |
...
`mcs validate` covers what an analyzer cannot see. The rest of the conventions — braces on every body,
dependencies named after their type, records for requests and results, one primary type per file, no
`DateTime.Now` — are compile errors, from
[MagicCSharp.Analyzers](../src/MagicCSharp.Analyzers/README.md), which the `Directory.Build.props` from
`mcs init` references for every project. The README lists the rules, MCS0001 to MCS0022.

Change a rule's severity, or turn it off, in `.editorconfig`:
...
The ambient-clock rule, MCS0008, overlaps `validate`'s: an exemption needs a
`#pragma warning disable MCS0008` as well as the `// conventions: allow` comment. `mcs init --no-build-rules`
creates a repository without them.
