...
public class ErrorHandlingTests
{
    ...
    [Fact]
    public void A_domain_conflict_is_a_409()
    {
        var problem = ErrorHandlingModule.Describe(
            new MagicCSharp.Infrastructure.Exceptions.EntityConflictException("Order 42 already exists."), false);

        Assert.Equal(409, problem.Status);
        Assert.Equal("Order 42 already exists.", problem.Detail);
    }

    [Fact]
    public void An_operation_the_entitys_state_refuses_is_a_422()
    {
        var problem = ErrorHandlingModule.Describe(
            new MagicCSharp.Infrastructure.Exceptions.EntityInvalidOperationException("Charge 7 is already paid."), false);

        Assert.Equal(422, problem.Status);
        Assert.Equal("Charge 7 is already paid.", problem.Detail);
    }
