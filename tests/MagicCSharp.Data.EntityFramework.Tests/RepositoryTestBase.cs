...
{
    protected WidgetsRepository Widgets => new WidgetsRepository(DbContextFactory, TimeProvider, NullLoggerFactory.Instance)
    {
        Ids = KeyGen,
    };

    protected ContactsRepository Contacts => new ContactsRepository(DbContextFactory, TimeProvider, NullLoggerFactory.Instance)
    {
        Ids = KeyGen,
    };

    protected ContactsSoftDeleteRepository DeletableContacts =>
        new ContactsSoftDeleteRepository(DbContextFactory, TimeProvider, NullLoggerFactory.Instance) { Ids = KeyGen };
