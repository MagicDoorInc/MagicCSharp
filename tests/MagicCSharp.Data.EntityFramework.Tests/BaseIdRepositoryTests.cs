...
public class BaseIdRepositoryTests : RepositoryTestBase
{
    ...
    {
        TimeProvider.SetUtcNow(new DateTimeOffset(2026, 3, 1, 12, 0, 0, TimeSpan.Zero));

        var created = await Widgets.Create(AWidget());

        Assert.Equal(TimeProvider.GetUtcNow(), created.Created);
        Assert.Equal(TimeProvider.GetUtcNow(), created.Updated);
    }
    ...
    public async Task Timestamps_come_back_as_utc()
    {
        ...
        TimeProvider.SetUtcNow(new DateTimeOffset(2026, 3, 1, 9, 0, 0, TimeSpan.FromHours(5)));

        var created = await Widgets.Create(AWidget());
        var read = await Widgets.Get(created.Id);

        Assert.Equal(TimeSpan.Zero, read!.Created.Offset);
        Assert.Equal(TimeProvider.GetUtcNow().ToUniversalTime(), read.Created);
    }
    ...
    public async Task Update_moves_the_updated_timestamp_and_leaves_created_alone()
    {
        ...
        Assert.Equal(TimeProvider.GetUtcNow(), updated.Updated);
    }
